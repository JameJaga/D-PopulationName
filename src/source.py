...
TOKEN = "NjAzOTQ3MDQ5MDc5MTQ0NDU4.XTqbmw.b39ehjg8-uOOSewHUnsYJv9JIUA" # トークンキー
TEXT_CHANNEL = 339370094704590858 # テキストチャットのチャンネルID
...
async def on_voice_state_update(before, after):
    voice_id = "369451341900152833"#which voice channel?
    text_id = "412484338450694144"#which text channel?
    v_ch = client.get_channel(voice_id)
...
async def on_message(message):
    ...
    if message.content.startswith('ynews'):
        news_id = '603954656271663116' #newsテキストチャネルID
    ...
    if message.content.startswith('nnews'):
        news_id = '603954878649335819' #newsテキストチャネルID
        news_ch = client.get_channel(news_id)
